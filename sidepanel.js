...
async function handleUserInput(input, existingUserMessageId = null) {
    ...
    try {
        // Send message to background while filler is playing
        const backgroundRequestStartTime = Date.now();
        console.log('=== SENDING TO BACKGROUND ===', new Date().toISOString());
        const response = await chrome.runtime.sendMessage({
            type: 'message',
            content: input
        });
        const backgroundRequestTime = Date.now() - backgroundRequestStartTime;
        console.log(`⏱️ Background API request took ${backgroundRequestTime}ms`);
